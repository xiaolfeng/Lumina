...
## 概述
`internal/app/startup/` 负责应用启动时的基础设施初始化（数据库、Redis、MCP Server）和幂等种子数据填充。
...
4. MCP 路由缺失 → 检查 `startup_mcp.go` 是否正确注册 `MCPHandlerKey`。
5. 种子数据异常 → 检查 `prepare/` 下对应文件，确认幂等逻辑。

## 执行顺序（不可更改）
1. `databaseInit`（注册为 `xCtx.DatabaseKey`）
2. `nosqlInit`（注册为 `xCtx.RedisClientKey`）
3. `mcpInit`（注册为 `MCPHandlerKey`）
4. `businessDataPrepare`（注册为 `xCtx.Exec`）
