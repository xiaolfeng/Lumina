...
// 将用户实体注入到 context.Context 中，后续 Handler 可通过 CtxUserKey 获取。
//
// 参数:
//   - ctx: 主上下文，用于依赖注入和日志
func Auth(ctx context.Context) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		// 验证访问令牌
		user, xErr := authLogic.ValidateAccessToken(c, accessToken)
		if xErr != nil {
		...
		// 注入用户到上下文
		newCtx := context.WithValue(c.Request.Context(), bConst.CtxUserKey, user)
