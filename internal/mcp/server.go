...
func InitMCPServer(ctx context.Context) http.Handler {
	server := mcp.NewServer(
		...
		&mcp.ServerOptions{
			Instructions: "Lumina 是 AI 代码认知与长期记忆的知识中枢。提供项目管理（project_create/get/list）、交互式 Q&A（qa_session_create/push_question/get_answer 等）、跨项目约束推送（pin_push/consume/list/update/peek）以及代码仓库 Wiki 分析（repoWiki_analyze/query/update/list/delete）能力。当用户需要管理项目注册、与用户进行富交互式问答、向其他项目传递依赖约束、或对代码仓库生成结构化 Wiki 文档时，使用这些工具。",
		},
	)
