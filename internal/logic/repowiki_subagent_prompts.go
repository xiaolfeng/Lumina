...
你将收到：
1. Coordinator 的项目概要
2. 多个 Explore Agent 的代码分析产出（每个产出都有一个唯一的 scope 标识）
...
## explore_refs 硬性约束（必须严格遵守）

**explore_refs 中每个元素必须逐字复制 user prompt 中 "可用的 Explore scope 列表" 里列出的 scope 字符串。**

禁止行为：
- 禁止为 scope 添加任何前缀（如 ` + "`explore-`" + ` ）
- 禁止对 scope 做语义化改写（如把 ` + "`internal_logic`" + ` 改成 ` + "`logic`" + `）
- 禁止自行编造未在列表中出现的 scope

正确示例：若可用 scope 列表为 ` + "`[\"internal_logic\", \"internal_handler\"]`" + `，则 ` + "`explore_refs: [\"internal_logic\"]`" + ` 合法，` + "`explore_refs: [\"explore-logic\"]`" + ` 非法。

每个页面条目的 explore_refs 至少关联 1 个 scope，最多 3 个。Writer 将严格按 explore_refs 检索参考资料，引用错误将导致 Writer 拿不到素材。`
...
func BuildArchitectUserPrompt(overviewSummary string, exploreOutputs []ExploreOutput) string {
	...
	// 显式列出所有可用 scope 标识 —— Architect 必须从此列表逐字选择 explore_refs
