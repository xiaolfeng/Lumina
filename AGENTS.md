...
基于 `bamboo-base-go` 构建的后端服务 + TanStack Start 前端，包含四大核心功能模块（设计阶段）：

- **RepoWiki**：克隆项目并通过 LLM 分析生成结构化 Wiki 文档
- **Memory**：AI 的长期决策记忆，MCP 端主动推送构建
- **Q&A**：Agent 与用户的富交互式问答通道（SSE 实时推送）
- **Pin**：跨项目依赖约束传递，点对点定向推送与 FIFO 队列消费

后端通过 Streamable MCP 协议 + HTTP REST API 双通道对外暴露能力；前端通过 REST API + SSE 与后端通信。前端构建产物通过 `go:embed` 嵌入 Go 二进制，支持单文件部署。
...
│   │   ├── route/              # 路由注册与中间件绑定（含前端 SPA fallback）
...
│   └── src/                    # 前端源码
│       ├── routes/             # 基于文件的路由（公开页 + 认证页 + 控制台）
│       ├── components/         # 组件（含 ui/ 和业务子目录 apikey/、project/）
│       ├── hooks/              # React Hooks（认证、API Key、项目）
...
| `NewRouteWithFrontend` | 函数 | `internal/app/route/route.go` | 全局中间件 + API 路由 + 前端 SPA fallback |
...
| `ApikeyLogic` | 结构体 | `internal/logic/apikey.go` | API Key 业务编排（创建/列表/更新/删除/重置/密钥生成/哈希/脱敏） |
...
| API Key | 密钥 CRUD/重置/脱敏展示 | Apikey 表（bcrypt 哈希） | ✅ 已实现 |
...
- **实体 ID 基因策略**：实体级别需绑定基因类型（`GeneProject = 32` 等），定义在 `constant/gene_number.go`。
...
- 认证模块已实现（登录、初始化、Token 刷新、Bearer 中间件）；RepoWiki/Memory/Q&A/Pin 尚在设计阶段。
- API Key 和项目模块已完整实现（后端 CRUD + 前端管理页面）。
- Pin 模块为新增设计模块（跨项目依赖约束传递），设计文档在 `docs/wiki/pin/`。
