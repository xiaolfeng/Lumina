...
export function WebhookConfig({ configId }: WebhookConfigProps) {
  ...
  if (!config) {
    return (
      <p className="text-sm text-muted-foreground">加载 Webhook 配置失败</p>
    )
  }

  const providers = Object.entries(config.provider_guide ?? {})
  const guideText =
    selectedProvider && config.provider_guide
      ? config.provider_guide[selectedProvider]
      : ''
    ...
    <>
      <div className="space-y-5">
        ...
        <div className="flex items-center gap-2 text-sm">
          ...
          <span
            className={
              config.has_secret ? 'text-foreground' : 'text-muted-foreground'
            }
          >
            {config.has_secret
              ? '已设置 Secret（HMAC 签名校验已启用）'
              : '未设置 Secret（建议配置以启用签名校验）'}
          </span>
        </div>
      ...
      <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              重新生成后，旧的 Token 和 Secret 将立即失效。所有使用旧凭据的
              Webhook 请求都会被拒绝。
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  onClick={() =>
                    handleCopy(
                      regenerateMutation.data?.data?.token ?? '',
                      '新 Token',
                    )
                  }
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  onClick={() =>
                    handleCopy(
                      regenerateMutation.data?.data?.secret ?? '',
                      '新 Secret',
                    )
                  }
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  onClick={() =>
                    handleCopy(
                      regenerateMutation.data?.data?.url ?? '',
                      '新 URL',
                    )
                  }
                  aria-label="复制新 URL"
