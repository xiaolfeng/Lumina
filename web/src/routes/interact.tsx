...
import { SessionProgressBar } from '#/components/interact/session-progress-bar'
import type { SessionProgress } from '#/hooks/useSidebarOpen'
import { SidebarOpenContext } from '#/hooks/useSidebarOpen'
    ...
    >
      <div className="flex h-screen flex-col bg-bg-base">
        ...
        <header className="flex h-12 shrink-0 items-center gap-2 border-b border-line bg-header-bg px-4">
          ...
          {progress && (
            <span className="inline-flex shrink-0 items-center gap-1 bg-emerald-50 px-2 py-0.5 text-[10px] font-medium text-emerald-600 mr-2">
          ...
          >
            <PanelRightOpen
              className={`size-4 transition-transform duration-200 ${sidebarOpen ? 'rotate-180' : ''}`}
              aria-hidden
            />
          </button>
        </header>
