...
export const Route = createFileRoute('/auth/new')({ component: NewPage })
...
function NewPage() {
  ...
  function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    initMutation.mutate(
      ...
      {
        onSuccess: async () => {
          // 使用 fetchQuery 强制重新请求 status，确保拿到最新的 is_initial 值
          // 不能用 invalidateQueries（仅标记 stale），否则 beforeLoad 的 ensureQueryData
          // 会返回旧缓存（is_initial=true），导致跳转到 /login 时被重定向回 /new
