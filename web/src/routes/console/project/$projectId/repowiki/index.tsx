...
export const Route = createFileRoute('/console/project/$projectId/repowiki/')({
  validateSearch: (search: Record<string, unknown>): { tab?: RepoWikiTab } => ({
      ...
      >
        <PageHeader
          ...
          action={
            <Button
              variant="outline"
              onClick={() => navigate({ to: '/console/project' })}
            >
        ...
        <motion.div variants={staggerItem}>
          <Card className="border-border bg-card">
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <Link
                to="/console/project/$projectId/repowiki/create"
                params={{ projectId }}
              >
        ...
        >
          ...
          <TabsContent value="versions" className="mt-0">
            <div className="rounded-lg border bg-card p-4">
              <VersionList
                configId={config.id}
                selectedVersionId={config.selected_version_id}
              />
            </div>
          </TabsContent>
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2">
          <Button
            variant="destructive"
            onClick={() => setCleanFailedOpen(true)}
            className="gap-2"
          >
        ...
        <div className="space-y-2">
          <Button
            variant="destructive"
            onClick={() => setKeepLatestOpen(true)}
            className="gap-2"
          >
        ...
        <div className="space-y-2">
          <Button
            variant="destructive"
            onClick={() => setCleanCacheOpen(true)}
            className="gap-2"
          >
          ...
          <p className="text-xs text-muted-foreground">
            删除本地 Git 克隆缓存，下次更新时将重新拉取。不影响已生成的 Wiki
            版本
          </p>
        </div>
      </div>
