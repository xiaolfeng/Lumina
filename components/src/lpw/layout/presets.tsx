...
export function renderLayoutPreset({
  ...
  const { pattern, direction = 'horizontal', strategy, gap = 'md', placements = [], alternateFrom = 'media' } = props
  const gapClass = GAP_MAP[gap] || 'gap-6'

  switch (pattern) {
    case 'split': {
      let gridColsStyle: React.CSSProperties | undefined
      let colsClass = 'grid-cols-1 md:grid-cols-2'

      if (strategy?.type === 'ratio' && strategy.tracks.length > 0) {
        colsClass = 'grid-cols-1 md:grid'
        gridColsStyle = {
          gridTemplateColumns: strategy.tracks.map((t) => `${t}fr`).join(' '),
        }
      ...
      return (
        <div
          data-testid="layout-split"
          className={`grid ${colsClass} ${gapClass}`}
          style={gridColsStyle}
        >
          {renderedChildren}
        </div>
    ...
    case 'bento': {
      ...
      const totalCols =
        strategy?.type === 'spans' ? strategy.columns : 3

      return (
        <div
          data-testid="layout-bento"
          className={`grid grid-cols-1 md:grid ${gapClass}`}
    ...
    case 'newspaper': {
      ...
      return (
        <div data-testid="layout-newspaper" className="space-y-8 font-sans">
          {/* eslint-disable-next-line @typescript-eslint/no-unnecessary-condition */}
          {leadNode ? <div className="border-b border-line pb-6">{leadNode}</div> : null}
    ...
    case 'editorial-wrap': {
      ...
      return (
        <div data-testid="layout-editorial-wrap" className="relative my-6 font-sans">
          {/* 桌面端浮动模式 */}
