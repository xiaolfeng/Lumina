...
function Splitter({
  ...
  const maxSizes = React.useMemo(
    () => panels.slice(0, -1).map((p) => p.props.maxSize ?? 100),
    [panels],
  );
  ...
  const handleResize = React.useCallback(
    ...
    [direction, sizes.length, minSizes, maxSizes],
  );
  ...
  const handleKeyResize = React.useCallback(
    ...
    [sizes.length, minSizes, maxSizes],
  );
