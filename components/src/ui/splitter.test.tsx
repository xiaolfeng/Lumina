...
function renderSplitter(props?: { direction?: "horizontal" | "vertical" }) {
  ...
  const panelA = container.querySelector<HTMLElement>(
    '[data-testid="panel-a"]',
  )!;
  const panelB = container.querySelector<HTMLElement>(
    '[data-testid="panel-b"]',
  )!;
...
describe("Splitter", () => {
  it("renders panels and handle with separator semantics", () => {
    ...
    expect(
      container.querySelectorAll('[data-slot="splitter-panel"]'),
    ).toHaveLength(2);
    expect(
      container.querySelectorAll('[data-slot="splitter-handle"]'),
    ).toHaveLength(1);
  ...
  it("renders vertical direction with horizontal separator orientation", () => {
    const { container, handle, splitter } = renderSplitter({
      direction: "vertical",
    });
    ...
    expect(
      container
        .querySelector('[data-slot="splitter"]')
        ?.classList.contains("grid-rows-[var(--splitter-track,1fr)]"),
    ).toBe(true);
  });
  ...
  it("honors explicit maxSize and clamps when dragged beyond", () => {
    const { container } = render(
      <Splitter data-testid="splitter">
        <SplitterPanel defaultSize={40} minSize={20} maxSize={50}>
          左
        </SplitterPanel>
        <SplitterHandle data-testid="handle" />
        <SplitterPanel minSize={20}>右</SplitterPanel>
    ...
    fireEvent.pointerMove(handle, { clientX: 700, clientY: 250 });
    fireEvent.pointerUp(handle);
