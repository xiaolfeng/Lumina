...
所有依赖项目上下文的业务模块（Q&A 会话、Preview 预览工作区等）均需要关联一个有效的 `project_id`（雪花 ID 字符串）。为了避免项目重复膨胀与无效创建，必须严格遵循以下标准流程进行项目解析。
...
① 获取工作区绝对路径 (pwd)
       │
       ▼
② project_get(match_path=pwd)
       │─── 命中 ───▶ [获取 project_id 结束]
       │
       ▼ 未直接命中
③ project_list(match_path=pwd)
       │─── 找到匹配项目 ───▶ [获取 project_id 结束]
       │
       ▼ 无任何匹配
④ project_create(name, match_path=[pwd])  [仅在当前任务明确需要时创建]
```
...
### 第一步：获取当前工作区绝对路径
执行命令（如 Unix/macOS 下的 `pwd` 或获取当前项目工作区根路径），确保拿到的是**标准化绝对路径**（例如 `/Users/username/projects/my-app`）。

### 第二步：优先调用 `project_get`
使用获取到的绝对路径作为 `match_path` 进行查询：
```json
{
...
若 `project_get` 未命中，调用 `project_list` 并传入 `match_path`：
```json
{
  "match_path": "/Users/username/projects/my-app"
}
...
**仅当**前两步均未找到已有项目，且**当前用户任务确实需要**建立长期会话或预览工作区时，才调用 `project_create` 注册项目：
```json
{
  "name": "my-app",
  "match_path": ["/Users/username/projects/my-app"],
...
3. **NEVER**: 严禁因为工具存在就无理由自动创建项目。只有当用户明确要求或任务确实需要持久化存储时才触发。
