...
│  │  RepoWiki   │  │   Memory    │  │      Q&A         │ │
│  │             │  │             │  │                  │ │
│  │ · Git Clone │  │ · 决策卡片  │  │ · Session 管理   │ │
│  │ · LLM 分析  │  │ · 标签分类  │  │ · 问题推送       │ │
│  │ · Wiki 生成 │  │ · Markdown  │  │ · 富交互渲染     │ │
│  │ · 增量同步  │  │ · 条件检索  │  │ · 答案收集       │ │
│  └──────┬──────┘  └──────┬──────┘  └────────┬─────────┘ │
│         │                │                  │            │
├─────────┴────────────────┴──────────────────┴────────────┤
...
| 原则 | 说明 |
|------|------|
| **三模块独立** | RepoWiki、Memory、Q&A 各自完整的 handler→logic→repository 分层，互不调用 |
| **MCP 编排** | Agent 通过 MCP 自行组合调用三个模块，Lumina 不做跨模块编排 |
