...
│                    对外接入层                                                │
│  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌──────────────────┐ │
│  │  Streamable │  │  HTTP REST  │  │     SSE     │  │     Webhook      │ │
│  │ MCP Server  │  │  API (Gin)  │  │(Q&A 实时推送)│  │    (RepoWiki)    │ │
│  └──────┬──────┘  └──────┬──────┘  └──────┬──────┘  └────────┬─────────┘ │
│         │                │                │                     │          │
├─────────┴────────────────┴────────────────┴──────────────────┴────────────┤
│                    模块层（四大独立领域）                    │
...
RepoWiki 模块额外暴露 Webhook 接收端点，供 Git Provider 在代码推送时主动触发 Wiki 增量更新。

| Provider | 认证方式 | 触发事件 |
|----------|----------|----------|
| GitHub | `X-Hub-Signature-256` HMAC-SHA256 | push |
| Gitee | `X-Gitee-Token` 静态 Token | Push Hook |
| GitLab | `X-Gitlab-Token` 静态 Token | push |
| Gitea | `X-Gitea-Signature` HMAC-SHA256 | push |

Webhook 端点不经过 Bearer Token 认证，仅通过 URL 中的 Token 与 Provider 签名进行校验。详细处理流程与配置方式参见 [repowiki/webhook-design.md](repowiki/webhook-design.md)。
